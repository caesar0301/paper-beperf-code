...
## One user for debugging
unique.uid <- unique(en.mob$UID)
macro.uid <- unique.uid[12010]
macro.uid <- "39856"
macro.user <- subset(en.mob, UID==macro.uid)
saveRDS(macro.user, paste("figures/hmm-model-user-",
                          macro.uid, ".rds", sep=""))
...
## Plot state series
pdf("figures/hmm-model-optimal-states-ts.pdf", width=5, height=3)
