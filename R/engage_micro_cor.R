...
attach(en.data)
## plot SDur ~ ADur
pdf("figures/engage-perc-sdur-adur.pdf", width=5, height=3)
...
## plot SDur ~ WT
pdf("figures/engage-perc-sdur-wt.pdf", width=5, height=3)
...
## plot SDur ~ PABW
pdf("figures/engage-perc-sdur-pbw.pdf", width=5, height=3)
mpar <- par(mar=c(4,4,1,1), cex.lab=1.5, mgp=c(2,0.5,0))
...
## plot IR ~ ADur
pdf("figures/engage-perc-ir-adur.pdf", width=5, height=3)
mpar <- par(mar=c(4,4,1,1), cex.lab=1.5, mgp=c(2,0.5,0))
...
## plot IR ~ WT
pdf("figures/engage-perc-ir-wt.pdf", width=5, height=3)
mpar <- par(mar=c(4,4,1,1), cex.lab=1.5, mgp=c(2,0.5,0))
...
## plot IR ~ PABW
pdf("figures/engage-perc-ir-pbw.pdf", width=5, height=3)
mpar <- par(mar=c(4,4,1,1), cex.lab=1.5, mgp=c(2,0.5,0))
...
## plot VF ~ ADur
pdf("figures/engage-perc-vf-adur.pdf", width=5, height=3)
mpar <- par(mar=c(4,4,1,1), cex.lab=1.5, mgp=c(2,0.5,0))
...
## plot VF ~ WT
pdf("figures/engage-perc-vf-wt.pdf", width=5, height=3)
mpar <- par(mar=c(4,4,1,1), cex.lab=1.5, mgp=c(2,0.5,0))
...
## plot VF ~ PABW
pdf("figures/engage-perc-vf-pbw.pdf", width=5, height=3)
mpar <- par(mar=c(4,4,1,1), cex.lab=1.5, mgp=c(2,0.5,0))
...
input.rds <- rep("rdata/engage.filtered.mob.rds",
                 times=length(output.rds))
structs <- list(c("UID"), c("service.en"),
                c("UID","service.en"))
mapply(function(ords, irds, struct){
    ...
    saveRDS(cor.mob, ords)}, output.rds, input.rds, structs)
...
##****************************##
##   Impacts of user factor   ##
##****************************##
## R({user}|{})
## plot significant users (mobile)
library(ggplot2)
usr.cor.mob <- readRDS("rdata/engage.user.cor.mob.rds")
uc.sig.m <- usr.cor.mob[which(usr.cor.mob$prob<0.05),
                        c("UID","dep","ind","cor")]
...
plot.user.cor <- function(df){
    ...
    ggsave("figures/engage-micro-cor-user.svg", width=8, height=3.5)}
...
plot.ulc.brank <- function(selector, filename){
    ulc.mob.selected <- ulc.mob[which(ulc.mob$eng == selector[1] &
                                          ulc.mob$perc == selector[2] &
                                              ulc.mob$prob < 0.05),
                                c("UID","BRank","eng","perc","cor")]
    uc.sig.m.selected <- uc.sig.m[which(uc.sig.m$eng == selector[1] &
                                            uc.sig.m$perc == selector[2]),]
